...
#define DEFAULT_FM_SAMPLE_RATE	12000
...
static void *controller_thread_fn(void *arg)
{
	...
	while (!do_exit) {
		...
		//TODO modded here
		//if(rtlsdr_set_center_freq(dongle.dev, dongle.freq) != -1)
		//	tuning_iface->tuned = TRUE;
...
int rtl_fm_setmode(enum mode_demod newMode) {
	...
	case FM:
		...
		demod2.mode_demod = &fm_demod;
		break;
	case RAW:
		demod.mode_demod = &raw_demod;
		demod2.mode_demod = &raw_demod;
		break;
	case AM:
		demod.mode_demod = &am_demod;
		demod2.mode_demod = &am_demod;
		break;
	case USB:
		demod.mode_demod = &usb_demod;
		demod2.mode_demod = &usb_demod;
		break;
	case LSB:
		demod.mode_demod = &lsb_demod;
		demod2.mode_demod = &lsb_demod;
		break;
	case WBFM:
	default:
		return -1;
	...
	while ((opt = getopt(argc, argv, "d:f:g:s:b:l:o:t:r:p:E:F:A:M:h")) != -1) {
		...
		case 'M':
			...
			if (strcmp("am",  optarg) == 0) {
				demod.mode_demod = &am_demod;}
			if (strcmp("usb", optar#include "loguru.hppg) == 0) {
				demod.mode_demod = &usb_demod;}
...
//TODO split point for init and deinit
