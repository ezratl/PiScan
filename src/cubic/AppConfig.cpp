...
std::string AppConfig::getConfigDir() {
    ...
    std::experimental::filesystem::path dataDir("./config");

    bool mkStatus = false;

    //if (!wxDir::Exists(dataDir)) {
    if (!std::experimental::filesystem::exists(dataDir)) {
        //mkStatus = wxDir::Make(dataDir);
        mkStatus = std::experimental::filesystem::create_directory(dataDir);
    } else {
        mkStatus = true;
    }
...
bool AppConfig::load() {
    ...
    std::experimental::filesystem::path cfgFile(cfgFileName);

    //if (!cfgFile.Exists()) {
    if (!std::experimental::filesystem::exists(cfgFile)) {
        if (configName.length()) {
            //wxFileName baseConfig = wxFileName(getConfigFileName(true));
            std::experimental::filesystem::path baseConfig(getConfigFileName(true));
            //if (baseConfig.Exists()) {
            if (std::experimental::filesystem::exists(baseConfig)) {
                //std::string baseConfigFileName = baseConfig.GetFullPath(wxPATH_NATIVE).ToStdString();
                std::string baseConfigFileName = baseConfig.root_path().generic_string();
                ...
                std::experimental::filesystem::copy(baseConfig, cfgFile);
                //if (!cfgFile.Exists()) {
                if (!std::experimental::filesystem::exists(cfgFile)) {
    ...
    if ((std::experimental::filesystem::status(cfgFile).permissions() & std::experimental::filesystem::perms::others_read) != std::experimental::filesystem::perms::none) {
