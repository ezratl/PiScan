...
int SDRThread::readStream(SDRThreadIQDataQueuePtr iqDataOutQueue) {
    ...
    while (n_read < nElems && !stopping) {
        ...
        if (n_stream_read == 0) {
             ...
             LOG_F(WARNING, "SDRThread::readStream(): 2. SoapySDR read blocking...");
             break;
        }
        else if (n_stream_read < 0) {
            //std::cout << "SDRThread::readStream(): 2. SoapySDR read failed with code: " << n_stream_read << std::endl;
            LOG_F(ERROR, "SDRThread::readStream(): 2. SoapySDR read failed with code: ");
            break;
        }
    ...
    if (n_read > 0 && !stopping && !iqDataOutQueue->full()) {
        ...
        if (!iqDataOutQueue->try_push(dataOut)) {
            ...
            LOG_F(WARNING, "SDRThread::readStream(): 3.2 iqDataOutQueue output queue is full, discard processing of the batch...");
...
void SDRThread::updateSettings() {
    ...
    if (rate_changed.load()) {
        ...
        if (!mtuElems.load()) {
            ...
            LOG_F(WARNING, "SDRThread: Device Stream MTU is broken, use %i instead...", mtuElems.load());
        } else {
            //std::cout << "SDRThread : Device Stream set to MTU: " << mtuElems.load() << std::endl << std::flush;
            LOG_F(INFO, "SDRThread : Device Stream set to MTU: %i", mtuElems.load());
        }
