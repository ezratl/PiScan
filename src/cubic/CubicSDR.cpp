...
int CubicSDR::OnExit() {
    ...
    LOG_F(INFO, "Terminating SDR thread");
    sdrThread->terminate();
    ...
    LOG_F(INFO, "Terminating SDR post-processing thread");
    ...
    if (!terminationSequenceOK) {
        ...
        LOG_F(WARNING, "Cannot terminate application properly, calling exit() now.");
        ::exit(12);
    }

    //std::cout << "Terminating All Demodulators.." << std::endl << std::flush;
    LOG_F(INFO, "Terminating all demodulators");
    ...
    LOG_F(INFO, "Demod backend termination complete");
...
static std::shared_ptr<CubicSDR> _cubicPtr;

std::shared_ptr<CubicSDR> makeCubic() {
    if(!_cubicPtr)
        _cubicPtr = std::make_shared<CubicSDR>();
    return _cubicPtr;
}
