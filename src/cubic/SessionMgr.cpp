...
void SessionMgr::saveSession(std::string fileName) {
    ...
    std::string tempversion(CUBICSDR_VERSION);
    header->newChild("version")->element()->set(std::wstring(tempversion.begin(), tempversion.end()));
    ...
    try {
        ...
        if (header->hasAnother("version")) {
            ...
            try {
                ...
                std::wcout << "Loading session file version: '" << version << "'..." << std::endl;
            }
                ...
                std::string tempversion(CUBICSDR_VERSION);
                version.assign(tempversion.begin(), tempversion.end());
            }
        }
